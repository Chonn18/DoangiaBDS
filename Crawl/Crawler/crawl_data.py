...
i = 1
count = 1
while QUEUE.qsize() > 0 and count <= 100:
    page_url = QUEUE.get()
    if page_url not in crawled:
        ...
        for link in detail_link:
            ...
            if len(d.return_data()) == data.shape[1]:
                ...
                with open(project_name+'/data.csv', 'a', encoding='utf-8-sig', newline='') as f:
                    w = writer(f)
                    if os.path.getsize(project_name+'/data.csv') == 0:
                        w.writerow(col)
        ...
        update_file(queue, crawled, project_name +
                    "/queue.txt", project_name+"/crawled.txt")
